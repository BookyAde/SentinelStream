...
async def ingest_batch(data: BatchEventIngest, db: AsyncSession = Depends(get_db)):
    """Ingest a batch of events. Maximum 500 events per request."""
    service = IngestionService(db)
...
async def list_events(
    ...
    Useful examples:
    - /api/v1/events?source=omsp_frontend
    - /api/v1/events?event_type=page_navigation
    - /api/v1/events?source=omsp_frontend&event_type=page_navigation
    """
    stmt = select(Event)
    ...
    if from_dt:
        filters.append(Event.created_at >= from_dt)
    if to_dt:
        filters.append(Event.created_at <= to_dt)
